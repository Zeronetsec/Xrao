...
func Runner(configPath string) {
    ...
    localScript := filepath.Join(localTmpDir, "xrao_reset.sh")
    sdcardScript := filepath.Join(variable.Sdx, "xrao_reset.sh")
    ...
    fmt.Printf(
        "%s[*] %sRemoving: %s/data/local/tmp/xrao_reset.sh %sif exist\n",
        color.B, color.N, color.GG, color.N,
    )
    _, _ = shell.Execf(
        "adb shell \"if [ -f /data/local/tmp/xrao_reset.sh ]; then rm /data/local/tmp/xrao_reset.sh; fi\"",
    )

    fmt.Printf(
        "%s[*] %sCopying: %s/sdcard/Download/xrao/xrao_reset.sh %s-> %s/data/local/tmp/%s\n",
        color.B, color.N, color.GG, color.DG, color.GG, color.N,
    )
    _, _ = shell.Execf(
        "adb shell \"cp %s/xrao_reset.sh /data/local/tmp/\"",
        variable.Sdx,
    )

    fmt.Printf(
        "%s[*] %sRemoving: %s/sdcard/Download/xrao/xrao_reset.sh%s\n",
        color.B, color.N, color.GG, color.N,
    )
    _ = os.Remove(sdcardScript)

    fmt.Printf(
        "%s[*] %sSet permission for: %s/data/local/tmp/xrao_reset.sh%s\n",
        color.B, color.N, color.GG, color.N,
    )
    _, _ = shell.Execf("adb shell \"chmod +x /data/local/tmp/xrao_reset.sh\"")

    fmt.Printf(
        "%s[*] %sCall: %s/system/bin/sh %s-> %s/data/local/tmp/xrao_reset.sh%s\n",
        color.B, color.N, color.GG, color.DG, color.GG, color.N,
    )
    execErr := shell.ExecLivef(
        "adb shell \"/system/bin/sh /data/local/tmp/xrao_reset.sh\"",
    )
    ...
    fmt.Printf(
        "%s[*] %sRemoving: %s/data/local/tmp/state.lock %sif exist\n",
        color.B, color.N, color.GG, color.N,
    )
    _, _ = shell.Execf(
        "adb shell \"if [ -f /data/local/tmp/state.lock ]; then rm /data/local/tmp/state.lock; fi\"",
    )
    ...
    fmt.Printf(
        "%s* %s/data/local/tmp/xrao_reset.sh%s\n",
        color.DG, color.GG, color.N,
    )
    _, _ = shell.Execf(
        "adb shell \"rm /data/local/tmp/xrao_reset.sh\"",
    )
