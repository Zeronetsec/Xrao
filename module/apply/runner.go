...
func Runner(configPath string) {
    ...
    localScript := filepath.Join(localTmpDir, "xrao_tweaks.sh")
    sdcardScript := filepath.Join(variable.Sdx, "xrao_tweaks.sh")
    ...
    fmt.Printf(
        "%s[*] %sRemoving: %s/data/local/tmp/xrao_tweaks.sh %sif exist\n",
        color.B, color.N, color.GG, color.N,
    )
    _, _ = shell.Execf(
        "adb shell \"if [ -f /data/local/tmp/xrao_tweaks.sh ]; then rm /data/local/tmp/xrao_tweaks.sh; fi\"",
    )

    fmt.Printf(
        "%s[*] %sCopying: %s/sdcard/Download/xrao/xrao_tweaks.sh %s-> %s/data/local/tmp/%s\n",
        color.B, color.N, color.GG, color.DG, color.GG, color.N,
    )
    _, _ = shell.Execf(
        "adb shell \"cp %s/xrao_tweaks.sh /data/local/tmp/\"",
        variable.Sdx,
    )

    fmt.Printf(
        "%s[*] %sSet permission for: %s/data/local/tmp/xrao_tweaks.sh%s\n",
        color.B, color.N, color.GG, color.N,
    )
    _, _ = shell.Execf(
        "adb shell \"chmod +x /data/local/tmp/xrao_tweaks.sh\"",
    )

    fmt.Printf(
        "%s[*] %sCall: %s/system/bin/sh %s-> %s/data/local/tmp/xrao_tweaks.sh%s\n",
        color.B, color.N, color.GG,
        color.DG, color.GG, color.N,
    )
    execErr := shell.ExecLivef(
        "adb shell \"/system/bin/sh /data/local/tmp/xrao_tweaks.sh\"",
    )
    ...
    fmt.Printf(
        "%s* %s/data/local/tmp/xrao_tweaks.sh%s\n",
        color.DG, color.GG, color.N,
    )
    _, _ = shell.Execf(
        "adb shell \"rm /data/local/tmp/xrao_tweaks.sh\"",
    )
